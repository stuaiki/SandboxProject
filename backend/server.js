  ...
  try {
    const completion = await client.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an assistant that helps users find popular places.',
        },
...
app.get('/places', async (req, res) => {
  ...
  if (!type || (type !== 'restaurant' && type !== 'tourist_attraction')) {
    return res.status(400).json({
      error: "Type must be either 'restaurant' or 'tourist_attraction'",
    });
  }
  ...
  if (latitude && longitude) {
    ...
    return res
      .status(400)
      .json({error: 'Either address or coordinates are required'});
  }
});
